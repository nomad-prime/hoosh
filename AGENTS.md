...
- Rust 1.75+ + okio (async runtime), serde_json (event serialization), existing LLM backends (Anthropic, OpenAI, TogetherAI, Ollama) (002-model-cascade)
- N/A (in-memory CascadeContext per request; no persistence in Phase 1) (002-model-cascade)
...
## Recent Changes
- 002-model-cascade: Added Rust 1.75+ + okio (async runtime), serde_json (event serialization), existing LLM backends (Anthropic, OpenAI, TogetherAI, Ollama)
