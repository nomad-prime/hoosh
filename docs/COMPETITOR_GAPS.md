...
- **3.3 Image input / screenshot support** — shipped this session, multimodal-only path. `ConversationMessage.attachments: Vec<Attachment>` (base64 in JSONL via custom serde, empty-skipped). New `LlmBackend::supports_images()` trait method: Anthropic hardcodes by model-name prefix (`claude-3*`, `claude-sonnet-*`, `claude-opus-*`, `claude-haiku-*`); OpenAI-compatible discovers via the existing `/models` fetch path (extended `ModelInfo.architecture.input_modalities`, OpenRouter shape). Anthropic native backend emits `ContentBlock::Image { source: { type: base64, media_type, data } }`; **openai-compatible backend** got a new `OpenAIWireMessage` shape with `content: Text(String) | Parts(Vec<{Text|ImageUrl}>)` because shipping `ConversationMessage` directly silently dropped attachments as a foreign JSON key (caught after first end-to-end test — model confabulated a plausible description from the `[pasted image-N]` marker). `Agent::handle_turn` bails before any backend traffic if any message carries attachments and `supports_images()` is false. `MessageParser` recognizes image extensions in `@` refs (.png/.jpg/.jpeg/.gif/.webp), loads bytes, replaces ref text with `[image #N]` marker. `ClipboardManager::get_image_png()` (RGBA → PNG via the `png` crate); Ctrl+V tries image first, falls back to text — paste creates an `ImageAttachment` on `AppState` and inserts `[pasted image-N]`. `KeyHandlerResult::StartConversation` carries the drained images, all three loops + `actions::answer()` plumb them through to `add_user_message_with_attachments`. Together/Ollama remain default-false. Deferred: image-token cost surfacing, queued-prompt image carry, non-multimodal sidecar describer (use the hard-error remediation hint instead). Deps: `base64`, `png`.
- **Retry message accuracy fix** — `src/backends/strategy.rs` displayed the count of completed failures rather than the upcoming attempt number, so users saw "Attempt 2/3" as the highest in-flight value even when all 3 attempts were burned. Now shows `Attempt {attempts+1}/{max_attempts}` (so 2/3 → 3/3 → "failed after 3 attempts" with no apparent gap). Also swapped the `{:?}` Duration debug format for a `format_duration` helper that renders `250ms` / `30s` / `2m 5s`.
- **3.1 `--no-session-persistence` polish** — flag was already plumbed from a prior session; added the missing `--continue` mutual-exclusion check to match acceptance, updated `docs/CLAUDE_CODE_PARITY.md` to mark Tier 3.1 done.
...
3.3 image input is now shipped for Anthropic native + OpenRouter (via openai-compatible). Sidecar-describer path for non-multimodal-only backends (Together/Ollama) is the natural follow-up.
