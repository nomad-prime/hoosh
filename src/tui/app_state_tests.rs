...
mod tests {
    ...
    fn completion_state_selected_item_returns_none_when_empty() {
        let state = CompletionState::new(0, 0);
        assert_eq!(state.selected_item(), None);
    }

    #[test]
    fn completion_state_selected_item_returns_correct_item() {
        let mut state = CompletionState::new(0, 0);
    ...
    fn app_state_new_initializes_correctly() {
        ...
        assert!(!state.autopilot_enabled.load(std::sync::atomic::Ordering::Relaxed));
    ...
    fn app_state_select_approval_options() {
        ...
        assert_eq!(state.approval_dialog_state.as_ref().unwrap().selected_index, 1);

        state.select_next_approval_option();
        assert_eq!(state.approval_dialog_state.as_ref().unwrap().selected_index, 0);
    }
