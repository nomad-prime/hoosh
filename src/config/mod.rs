...
impl AppConfig {
    ...
    pub fn update_backend_setting(&mut self, backend_name: &str, key: &str, value: String) -> Result<()> {
        let config = self.backends.entry(backend_name.to_string()).or_insert_with(|| BackendConfig {
            api_key: None,
            model: None,
            base_url: None,
        });
